  ...
  def create
    if current_user.nil?
      ...
      if formatted_plus_4 >= @reservation.date && current_user.standard?
        flash[:alert] = "Reservation was not Saved! Online reservations must be made by no later than 8pm on the day before the activity. Also, no online reservation dates
